...
name = 'ortho_new_vue1'
path_satellite = 'Pleiades_Vue1_2023\C1_orthoimage_forward.tif'
...
def concatenatation_ortho_vue123(output_path , path_v1, path_v2, path_v3):
    ...
    back = sorted(glob.glob(path_v1), key=os.path.getctime)
    nadir = sorted(glob.glob(path_v2), key=os.path.getctime)
    forw = sorted(glob.glob(path_v3), key=os.path.getctime)
...
extract_images_(path_shp, path_tif, output = f"C:/Users/rahim/Deeplearning_oct_2024/DeepLearning_EPHEMERE_Tree/Programme_DL/YOLO_SEMI_supervised/image_set_{name}/")
...
#orthoimage_to_png_opencv( f'./{name}/image_unlabel_{name}/*', f'./{name}/png_unlabel_{name}/')
#orthoimage_to_png_opencv( f'./train_image_{name}/*', f'./png_train_image_{name}/')

# pansharpen_to_png(image_pan_tif = f'./{name}/image_unlabel_{name}/*', ouput_png_3bds = f'./{name}/png_unlabel_{name}/')
# pansharpen_to_png(image_pan_tif = f'./{name}/train_image_{name}/*', ouput_png_3bds = f'./{name}/png_train_{name}/')
# pansharpen_to_png(image_pan_tif = f'./{name}/test_image_{name}/*', ouput_png_3bds = f'./{name}/png_test_{name}/')




###ON CONCATENE 
# concatenatation_ortho_vue123(output_path= './ortho_full/train/', path_v1 ='./ortho_vue1/png_train_image_ortho_vue1/*' , 
#                              path_v2 ='./ortho_vue2/png_train_image_ortho_vue2/*', path_v3 = './ortho_vue3/png_train_image_ortho_vue3/*')

#concatenatation_ortho_vue123(output_path= './ortho_full/unlabel/', path_v1 ='./ortho_vue1/png_unlabel_ortho_vue1/*' , 
#                            path_v2 ='./ortho_vue2/png_unlabel_ortho_vue2/*', path_v3 = './ortho_vue3/png_unlabel_ortho_vue3/*')
