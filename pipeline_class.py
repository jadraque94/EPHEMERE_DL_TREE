...
class YoloProcessor:
    ...
    def extract_images(self, path_tif)-> tuple[list, dict] : # we will store the small images cutting by the grid in image_array and store the metadata and corner associated at each image and the
        ...
        with rasterio.open(path_tif) as src:
            for idx, row in tqdm(grid.iterrows()):
                ...
                if out_image.shape[2] > 3 : #multispecral case
                    ...
                    band_normalized = cv2.normalize(out_image, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U).astype(np.uint8) # pyright: ignore[reportArgumentType, reportCallIssue]
    ...
    def separate_grid_unlabel_label(self, array, dicti) -> tuple[ list, pd.DataFrame, list, dict, dict ] : #we will separate image regarding as there are any wrosnw of tree which has been delineated and store in unlabel or label dataset
    ...
    def train_test_split(self, gdf, image_label, dicti) -> tuple[np.ndarray, np.ndarray, dict, dict, pd.DataFrame, pd.DataFrame]:
    ...
    @staticmethod
    def create_folder_txt(label, direction, output): # for creating folder for all of the label file 
    ...
    @staticmethod
    def create_folder_image(image, direction, dicti_image, number_band, output_image): # for creating folder for all of the label file 
